================================================================================
    MFC (MICROSOFT FOUNDATION CLASS) ライブラリ : ControlUnitAppProto プロジェクトの概要
===============================================================================

この ControlUnitAppProto アプリケーションは、アプリケーション ウィザードによって作成されました。このアプリケーションは MFC の基本的な使い方を示すだけでなく、アプリケーションを作成するための開始点でもあります。

このファイルには、ControlUnitAppProto アプリケーションを構成する各ファイルの内容の概要が含まれています。

ControlUnitAppProto.vcxproj
...
ControlUnitAppProto.vcxproj.filters
...
ControlUnitAppProto.h
    これはアプリケーションのメイン ヘッダー ファイルです。
    その他のプロジェクト固有のヘッダー (Resource.h など) が含まれ、CControlUnitAppProtoApp アプリケーション クラスを宣言します。

ControlUnitAppProto.cpp
    これはメインのアプリケーション ソース ファイルで、CControlUnitAppProtoApp アプリケーション クラスが含まれています。

ControlUnitAppProto.rc
...
res\ControlUnitAppProto.ico
    これはアプリケーションのアイコンとして使用されるアイコン ファイルです。このアイコンは、メイン リソース ファイル ControlUnitAppProto.rc にインクルードされます。

res\ControlUnitAppProto.rc2
...
res\Toolbar.bmp
    ...
    初期ツール バーと初期ステータス バーは、CMainFrame クラスで生成されます。ツール バーのボタンを追加するには、リソース エディターを使用してこのツール バーのビットマップを編集し、ControlUnitAppProto.rc 内の IDR_MAINFRAME TOOLBAR 配列を更新します。
/////////////////////////////////////////////////////////////////////////////
...
ControlUnitAppProtoDoc.h, ControlUnitAppProtoDoc.cpp - ドキュメント
    これらのファイルには、CControlUnitAppProtoDoc クラスが含まれます。これらのファイルを編集して、特殊なドキュメント データを追加し、ファイルの保存と読み込みを実装することができます (CControlUnitAppProtoDoc::Serialize を使用)。

ControlUnitAppProtoView.h, ControlUnitAppProtoView.cpp - ドキュメントのビュー
    これらのファイルには、CControlUnitAppProtoView クラスが含まれます。
    CControlUnitAppProtoView オブジェクトは CControlUnitAppProtoDoc オブジェクトを表示するために使用されます。

res\ControlUnitAppProtoDoc.ico
    これは、CControlUnitAppProtoDoc クラスの MDI 子ウィンドウのアイコンとして使用されるアイコン ファイルです。このアイコンは、メイン リソース ファイル ControlUnitAppProto.rc にインクルードされます。
...
StdAfx.h, StdAfx.cpp
    これらのファイルは、ControlUnitAppProto.pch という名前のプリコンパイル済みヘッダー (PCH) ファイルと、StdAfx.obj という名前のプリコンパイル済みの型ファイルをビルドするために使用されます。

Resource.h
    これは、新しいリソース ID を定義する標準のヘッダー ファイルです。このファイルの読み込みおよび更新は、Microsoft Visual C++ で行います。

ControlUnitAppProto.manifest
