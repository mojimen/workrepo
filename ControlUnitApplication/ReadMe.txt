================================================================================
    MFC (MICROSOFT FOUNDATION CLASS) ライブラリ : ControlUnitApplication プロジェクトの概要
===============================================================================

この ControlUnitApplication アプリケーションは、アプリケーション ウィザードによって作成されました。このアプリケーションは MFC の基本的な使い方を示すだけでなく、アプリケーションを作成するための開始点でもあります。

このファイルには、ControlUnitApplication アプリケーションを構成する各ファイルの内容の概要が含まれています。

ControlUnitApplication.vcxproj
...
ControlUnitApplication.vcxproj.filters
...
ControlUnitApplication.h
    これはアプリケーションのメイン ヘッダー ファイルです。
    その他のプロジェクト固有のヘッダー (Resource.h など) が含まれ、CControlUnitApplicationApp アプリケーション クラスを宣言します。

ControlUnitApplication.cpp
    これはメインのアプリケーション ソース ファイルで、CControlUnitApplicationApp アプリケーション クラスが含まれています。

ControlUnitApplication.rc
...
res\ControlUnitApplication.ico
    これはアプリケーションのアイコンとして使用されるアイコン ファイルです。このアイコンは、メイン リソース ファイル ControlUnitApplication.rc にインクルードされます。

res\ControlUnitApplication.rc2
...
res\Toolbar.bmp
    ...
    初期ツール バーと初期ステータス バーは、CMainFrame クラスで生成されます。ツール バーのボタンを追加するには、リソース エディターを使用してこのツール バーのビットマップを編集し、ControlUnitApplication.rc 内の IDR_MAINFRAME TOOLBAR 配列を更新します。
/////////////////////////////////////////////////////////////////////////////

アプリケーション ウィザードは 1 つのドキュメントの種類と 1 つのビューを作成します。

ControlUnitApplicationDoc.h, ControlUnitApplicationDoc.cpp - ドキュメント
    これらのファイルには、CControlUnitApplicationDoc クラスが含まれます。これらのファイルを編集して、特殊なドキュメント データを追加し、ファイルの保存と読み込みを実装することができます (CControlUnitApplicationDoc::Serialize を使用)。

ControlUnitApplicationView.h, ControlUnitApplicationView.cpp - ドキュメントのビュー
    これらのファイルには、CControlUnitApplicationView クラスが含まれます。
    CControlUnitApplicationView オブジェクトは CControlUnitApplicationDoc オブジェクトを表示するために使用されます。
...
StdAfx.h, StdAfx.cpp
    これらのファイルは、ControlUnitApplication.pch という名前のプリコンパイル済みヘッダー (PCH) ファイルと、StdAfx.obj という名前のプリコンパイル済みの型ファイルをビルドするために使用されます。

Resource.h
    これは、新しいリソース ID を定義する標準のヘッダー ファイルです。このファイルの読み込みおよび更新は、Microsoft Visual C++ で行います。

ControlUnitApplication.manifest
